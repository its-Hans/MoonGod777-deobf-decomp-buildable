...
public class TestUtil {
   ...
   public static List<Block> emptyBlocks = Arrays.asList(
      Blocks.AIR,
      Blocks.FLOWING_LAVA,
      Blocks.LAVA,
   ...
   public static List<Block> rightclickableBlocks = Arrays.asList(
      Blocks.CHEST,
   ...
   public static boolean placeBlock(BlockPos pos) {
      if (isBlockEmpty(pos)) {
         for(EnumFacing f : EnumFacing.values()) {
            ...
            Vec3d vec = new Vec3d(
               (double)pos.getX() + 0.5 + (double)f.getXOffset() * 0.5,
               (double)pos.getY() + 0.5 + (double)f.getYOffset() * 0.5,
               (double)pos.getZ() + 0.5 + (double)f.getZOffset() * 0.5
            );
            if (!emptyBlocks.contains(neighborBlock) && mc.player.getPositionEyes(mc.getRenderPartialTicks()).distanceTo(vec) <= 4.25) {
               ...
               mc.player
                  .connection
                  .sendPacket(new CPacketPlayerTryUseItemOnBlock(pos.offset(f), f.getOpposite(), EnumHand.MAIN_HAND, 0.0F, 0.0F, 0.0F));
               if (rightclickableBlocks.contains(neighborBlock)) {
