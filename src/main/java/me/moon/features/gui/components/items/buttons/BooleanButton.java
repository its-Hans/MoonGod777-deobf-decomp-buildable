...
public class BooleanButton extends Button {
   ...
   public Setting<Boolean> itSetting() {
      return setting;
   }
   ...
   public void drawScreen(int mouseX, int mouseY, float partialTicks) {
      ...
      Moon.textManager
         .drawStringWithShadow(this.getName(), this.x + 2.3F, this.y - 1.7F - (float)MoonGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
   }
   ...
   @Override
   public void toggle() {
      this.setting.setValue(!itSetting().getValue());
   }

   @Override
   public boolean getState() {
      return itSetting().getValue();
   }
}
